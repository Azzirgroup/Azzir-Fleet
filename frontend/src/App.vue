  ...
  <div class="flex h-screen w-screen overflow-hidden bg-gray-50 text-gray-900">
    ...
    >
      ...
      <div class="border-t border-white/10 px-3 py-3">
        <div class="px-1 pb-2 text-xs text-white/60">
          {{ user }}
        </div>

        <button
          type="button"
          class="flex w-full items-center gap-2 rounded-md px-3 py-2 text-sm text-white/80 hover:bg-white/10 hover:text-white"
          @click="logout"
        >
          <span>🚪</span>
          <span>Logout</span>
...
const portalOnly =
  window.portal_only ??
  window.frappe?.boot?.portal_only ??
  false
